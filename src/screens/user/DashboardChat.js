...
function DashboardChat({navigation}) {
  ...
  const getConversations = async () => {
    ...
    axios
      .get(`http://${IP}:8000/api/conversation/getByUser/${id}`)
      .then(async res => {
  ...
  const Item = ({driver, user, messages, idConversation}) => (
    <TouchableOpacity
      onPress={() =>
      ...
      <View style={styles.item}>
        <View style={styles.firstPart}>
          <Image
            source={{uri: `http://${IP}:8000/images/${driver?.personalImage}`}}
            style={styles.itemImage}
          />
          <View>
            <Text style={styles.itemName}>{driver.fullName}</Text>
